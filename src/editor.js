...
async function onInsertImage(data, callback) {
	...
	// const contents = (await deferredToPromise(fileClient.getFileContents(path)))[1]
	// 'invalid or missing file' error
	const blob = await loadFileContent(path, 'image/png')
	
	
	// const shares = await getShares(path, false)
	// const shareUrl = findShareUrl(shares)
	// direct link - does not work
	// const blob = await getImage('http://localhost:8080/index.php/f/113')

	//	example image binary data - this works?
